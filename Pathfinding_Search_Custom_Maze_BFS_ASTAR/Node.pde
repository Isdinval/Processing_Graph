...
class Node {
  ...
  float f_ASTAR = 0;
  float g_ASTAR = 0;
  float heuristic_ASTAR = 0;

  // GreedyBFS
  float heuristic_GreedyBFS = 0;
  ...
  void addNeighbors(Node[][] grid, boolean allow_diagonale) {
  ...
  void removeNeighbors(Node[][] grid ) {
    if (x < cols - 1 && visited == false) { //1 
      neighbors.remove(grid[x+1][y]);
    }
    if (x > 0 && visited == false) { //2
      neighbors.remove(grid[x-1][y]);
    }
    if (y > 0 && visited == false) { //3
      neighbors.remove(grid[x][y-1]);
    }
    if (y < rows - 1 && visited == false) { //4
      neighbors.remove(grid[x][y+1]);
    }
