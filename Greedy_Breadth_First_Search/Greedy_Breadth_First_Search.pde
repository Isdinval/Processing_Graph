...
//*********************************************************
// Affiche les murs du labyrinthe, chaque mur possède 45% de 
// chance de l'être. De plus, affiche la valeur de la fonction
// heuristique pour chaque noeud (sous condition) !!!
void showWall() {
  for (int i = 0; i < cols; i++) {
    for (int j = 0; j < rows; j++) {
      grid[i][j].show();
      if (cols < 35 && rows < 35) {
        fill(0);
        textSize(15);
        text((int)grid[i][j].heuristic, i*w + w/2, j*h + h/2);
      }
    }
  }
}
...
void BFS() {
  if (openSet.size() > 0) {
    ...
    int winner = 0;
    for (int i = 0; i < openSet.size(); i++) {
      if (openSet.get(i).heuristic< openSet.get(winner).heuristic) {
        winner = i;
      }
    }
    ...
    for (int i = 0; i < neighbors.size(); i++) {
      ...
      if (newPath) {
        ...
        neighbor.heuristic = heuristic(current, end);
      }
    }
  } else {
